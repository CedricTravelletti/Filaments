...
filament_inds, filament_vals = mygp.compute_filament_criterion(
        gradient, hessian, 1e-1)

# Plot results.
norm[:] = 0.0
norm[filament_inds] = 10.0
norm_2d = simulation_grid.reshape_to_2d(norm)
diff = np.abs(G - norm_2d.numpy())
N = diff/diff.max()  # normalize 0..1
...
surf = ax.plot_surface(
            X, Y, vals_2d, rstride=1, cstride=1,
                facecolors=cm.jet(N), alpha=1.0)

plt.show()
"""
filament_cells = simulation_grid.cells_list[filament_inds, :]
ax.scatter(filament_cells[:, 1], filament_cells[:, 0],
        sim_points_vals[filament_inds], s=100)
"""
